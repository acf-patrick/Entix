...
public:
	...
	*/
	...
	/**
	 * @param key Associated key to the callback. Used to indentify the function when removal is needed
	 * @param callback Change made on this Observable will trigger the callback
	*/
	virtual void submit(const std::string &key, const std::function<void(T &)> &callback)
	{
	...
	*/
	void distach(const std::string &key)
	{
...
public:
	...
	void submit(const std::string &key, const std::function<void(T &)> &callback) override
	{
