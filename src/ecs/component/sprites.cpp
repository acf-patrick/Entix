...
void spriteRenderer::Render() {
  if (!has<sprite>()) {
    std::cerr << "SpriteRenderer-error : Entity must have a sprite component "
                 "to render!"
              << std::endl;
    return;
  }
  ...
  Renderer->submit([&](SDL_Renderer* renderer) {
    ...
    // compute source rect
    src.x +=
        (spriteComponent.frame % spriteComponent.framesNumber.x) * frameSize.x;
    src.y +=
        (spriteComponent.frame / spriteComponent.framesNumber.x) * frameSize.y;
    src.w = frameSize.x;
    src.h = frameSize.y;
